...
unsigned LoopMeshBuilder::marchCubes(const ParametricScalarField &field)
{
    ...
    #pragma omp parallel shared(totalCubesCount, field, totalTriangles) private(i)
    {
        // 2. Loop over each coordinate in the 3D grid.
        #pragma omp for schedule(dynamic, 32) nowait reduction(+:totalTriangles)
        for(i = 0; i < totalCubesCount; ++i)
        {
